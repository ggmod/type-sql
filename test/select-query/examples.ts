...
export default (db: QuerySource, log: TestLog) => {
    ...
    describe('Random examples', () => {
        it('basic queries', sync(async () => {
            ...
            let s1 = `SELECT "Book"."id", LOWER("Book"."title") FROM "Book" WHERE "Book"."author" = 'xy' ORDER BY "Book"."title" ASC NULLS LAST OFFSET 10 LIMIT 2`;
            ...
            let s2 = `SELECT COUNT("Book".*), "Book"."author" FROM "Book" WHERE "Book"."price" > 100 GROUP BY "Book"."author" HAVING COUNT("Book".*) > 10 OR "Book"."author" LIKE 'X%' ORDER BY COUNT("Book".*) DESC, "Book"."author" ASC`;

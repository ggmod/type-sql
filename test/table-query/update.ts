...
describe('UPDATE', () => {
    ...
    it('conditions', () => {
        ...
        expect(db.sql).toEqual(`UPDATE "Book" SET "title" = 'qwe' WHERE "Book"."title" = 'asd'`);

        db.table(BOOK).where(BOOK.title.eq('asd'), BOOK.price.lt(100)).update(e);
        expect(db.sql).toEqual(`UPDATE "Book" SET "title" = 'qwe' WHERE "Book"."title" = 'asd' AND "Book"."price" < 100`);
    ...
    it('all', () => {
        ...
        expect(db.sql).toEqual(`UPDATE "Book" SET "title" = 'qwe'`);
    });
    ...
    it('multiple field types', () => {
        ...
        db.table(BOOK).updateAll(e);
        expect(db.sql).toEqual(`UPDATE "Book" SET "available" = TRUE, "date" = '2016-10-23T19:11:25.342Z', "price" = 10, "title" = 'asd'`);
    });
    ...
    it('json fields', () => {
        ...
        db.table(BOOK).updateAll(e);
        expect(db.sql).toEqual(`UPDATE "Book" SET "data" = '{"x":2,"y":10}'`);
    });
});

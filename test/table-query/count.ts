...
export default (db: QuerySource, log: TestLog) => {
    ...
    describe('Count', () => {
        ...
        it('conditions', sync(async () => {
            ...
            expect(log.sql).toEqual(`SELECT COUNT(*) FROM "Book" WHERE "Book"."title" = 'asd'`);

            await db.table(BOOK).where(BOOK.title.eq('asd'), BOOK.price.lt(100)).count();
            expect(log.sql).toEqual(`SELECT COUNT(*) FROM "Book" WHERE "Book"."title" = 'asd' AND "Book"."price" < 100`);
